...
def index(request: Request):
    ...
    return templates.TemplateResponse(
        request, "index.html", {"title": "Home"} | countries
    )
...
def country(
    ...
    try:
        ...
        return templates.TemplateResponse(
            request, "country.html", {"title": "Country"} | country
        )


@app.get("/countries", response_class=HTMLResponse)
def country_list(request: Request, query: Annotated[FilterBase, Query()]):
    country_model = Country()
    try:
        ...
        return templates.TemplateResponse(
            request, "country_list.html", {"title": "Countries"} | entity
        )
...
def author(
    ...
    try:
        entity = author.get_author(
            id, result_type=query.result_type, skip=query.skip, limit=query.limit
        )
    except KeyError:
        raise HTTPException(status_code=404, detail=f"Author '{id}' not found")
    else:
        return templates.TemplateResponse(
            request, "author.html", {"title": "Author"} | entity  # Merges dicts
        )


@app.get("/authors", response_class=HTMLResponse)
def author_list(request: Request, query: Annotated[FilterWorkstream, Query()]):
    authors = Author()
    try:
        entity = authors.get_authors(
            skip=query.skip, limit=query.limit, workstream=query.workstream
        )
    except KeyError as ex:
        raise HTTPException(status_code=404, detail=f"Authors not found")
    else:
        return templates.TemplateResponse(
            request, "authors.html", {"title": "Author List"} | entity  # Merges dicts
        )
...
def output_list(request: Request, query: Annotated[FilterOutputList, Query()]):
    ...
    try:
        ...
        return templates.TemplateResponse(
            request, "outputs.html", {"title": "Output List"} | package
        )


@app.get("/outputs/{id}", response_class=HTMLResponse)
def output(
    request: Request, id: Annotated[UUID, Path(title="Unique output identifier")]
):
    output_model = Output()
    try:
        ...
        return templates.TemplateResponse(
            request, "output.html", {"title": "Output"} | entity
        )
...
def workstream_list(request: Request):
    ...
    try:
        ...
        try:
            ...
            return templates.TemplateResponse(
                request, "workstreams.html", {"title": "Workstream"} | entity | all
            )


@app.get("/workstreams/{id}", response_class=HTMLResponse)
def workstream(request: Request, id: str, query: Annotated[FilterBase, Query()]):
    model = Workstream()
    all = model.get_all()
    try:
        ...
        return templates.TemplateResponse(
            request, "workstreams.html", {"title": "Workstreams"} | entity | all
        )
